...
6. **仪表盘** Tab：连续上传 2 天以上总览后，应出现资产与当日收益折线；持仓分布条与最新快照一致。
7. 校对表：符号矛盾行高亮；有历史快照时出现「沿用上次基金列表」。

### 每日快照（2026-06-01）

总览 OCR 成功后写入 `portfolio_daily_snapshots`（按自然日 upsert）。用于仪表盘走势、与上次持仓 diff、校对警告。
