...
**更新记录：**
- **主题板块扩展 ~100 + 后台刷新（2026-06-18）：** `GET /api/market/theme-boards` 板块全集从 21 扩到 **行业全量(~86) + 21 canonical 概念/指数主题**（`list_theme_board_universe()` 按 secid 去重，每行带 `board_kind`：industry/concept/index）；**涨跌幅与连涨天数统一从同一段 push2delay 日 K 计算**（`refresh_theme_board_snapshot` 并行 ~100 板块、90s 预算），日 K 缺失按 BK 代码取现货榜兜底；移除 `linked_fund_count` 列/字段；持仓高亮按 canonical secid 精确匹配。**后台 daemon 线程时段感知刷新**（lifespan 启动、盘中 15min / 收盘 1h，env `FUND_AI_THEME_BOARD_REFRESH_*`，CI 关闭），接口与前端只读缓存（`theme:boards:v3`、前端 staleTime 15min）；响应新增 `refreshed_at`。前端 `ThemeSectorOverview` 加 行业/概念/指数 标签、更新时间用 `refreshed_at`。设计见 `docs/superpowers/specs/2026-06-18-market-theme-boards-expansion-design.md`。
...
| GET | `/api/market/theme-boards` | 主题板块（**行业全量 ~86 + 21 canonical 概念/指数 ≈ 100**）：默认 `sort=change`（日涨幅降序）；每行含 `board_kind`（industry/concept/index）；**涨跌幅 + 连涨天数同源 push2delay 日 K**（后台 `refresh_theme_board_snapshot` 写缓存）；日 K 缺失按 BK 代码取现货榜兜底；含我的持仓高亮（canonical secid 匹配）；响应含 `refreshed_at`；后台 daemon 15min/1h 刷新、前台只读缓存；`force_refresh` 强制刷新 |
