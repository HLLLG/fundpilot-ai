...
**文档版本：** 2026-06-18（主题板块扩展 ~100 · 后台 15min 刷新）

**更新记录：**
- **主题板块扩展 ~100 + 后台刷新（2026-06-18）：** `GET /api/market/theme-boards` 板块全集从 21 扩到 **canonical 21 概念/指数（全保留）+ 东财行业板块按当日涨幅取前列**，合计约 **100**（`list_theme_board_universe(max_boards=100)`；东财 `m:90 t:2` 实返 ~496 细分行业，按涨幅截断到主榜量级、对齐小倍 ~76）；每行带 `board_kind`（industry/concept/index）。**涨跌幅与连涨天数统一从同一段 push2delay 日 K 计算**（`refresh_theme_board_snapshot` 并行、120s 预算、`allow_akshare=False` 跳过 per-board 子进程兜底），日 K 缺失用行业 spot f3（`change_hint`）兜底涨跌幅、连涨显示 `—`；移除 `linked_fund_count`；持仓按 canonical secid 精确高亮。**板块列表 clist 抓取改 push2delay 优先**（`eastmoney_spot_client._CLIST_HOSTS`，push2 数字子域偶发 Server disconnected 时回落），同时修复全市场子 Tab。**后台 daemon 线程时段感知刷新**（lifespan 启动、盘中 15min / 收盘 1h，env `FUND_AI_THEME_BOARD_REFRESH_*`，CI 关闭），接口与前端只读缓存（`theme:boards:v3`、前端 staleTime 15min）；响应新增 `refreshed_at`。前端 `ThemeSectorOverview` 加 行业/概念/指数 标签、更新时间用 `refreshed_at`。设计见 `docs/superpowers/specs/2026-06-18-market-theme-boards-expansion-design.md`。
...
| 市场板块 | **市场** Tab：`MarketTab` — 子 Tab「全市场 \| 主题板块 \| 美股」；全市场：`SectorPerformanceCard` + `HotSectorList`；主题：`ThemeSectorOverview`（`GET /api/market/theme-boards`，**行业全量+canonical ≈ 100 板块**、`board_kind` 标签、涨幅+连涨同源 **push2delay 日 K**；后台 15min 刷新、前台只读缓存；小倍式涨幅榜 UI） |
...
| GET | `/api/market/theme-boards` | 主题板块（**canonical 21 + 行业按涨幅 top，合计 ~100**）：默认 `sort=change`；每行含 `board_kind`（industry/concept/index）；**涨跌幅 + 连涨天数同源 push2delay 日 K**（后台 `refresh_theme_board_snapshot` 写缓存、`allow_akshare=False`）；日 K 缺失用行业 spot f3 兜底涨跌幅；含我的持仓高亮（canonical secid）；响应含 `refreshed_at`；后台 daemon 15min/1h 刷新、前台只读缓存；clist 抓取 push2delay 优先；`force_refresh` 强制刷新 |
...
18. 改市场板块：`sector_board_snapshot.py` / `theme_board_snapshot.py` / `sector_daily_kline_provider.py` → `main.py` `/api/market/*` → `MarketTab.tsx` / `api.ts` / `marketSectorBoard.ts` / `marketThemeBoard.ts` → `tests/test_sector_board_snapshot.py` / `test_theme_board_snapshot.py`；设计见 `2026-06-17-market-*-design.md`、`2026-06-18-market-theme-boards-expansion-design.md`。主题板块后台刷新线程在 `theme_board_snapshot.theme_board_refresh_loop`，由 `lifespan.py` 启动（env `FUND_AI_THEME_BOARD_REFRESH_*`）；板块全集 `list_theme_board_universe()`，刷新 `refresh_theme_board_snapshot()`（缓存 `theme:boards:v3`）。
...
| `docs/superpowers/specs/2026-06-18-market-theme-boards-expansion-design.md` | 市场 Tab — 主题板块扩展（~100 板块 + 后台 15min 刷新） |
