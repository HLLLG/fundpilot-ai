...
**文档版本：** 2026-06-22（移除简报 Tab + 持仓删除 + apply/OCR/板块修复）
...
18. 改市场板块：`sector_board_snapshot.py` / `theme_board_snapshot.py` / `dip_radar_snapshot.py` / `sector_daily_kline_provider.py` → `main.py` `/api/market/*` → `MarketTab.tsx` / `DipReboundRadar.tsx` / `api.ts` / `marketThemeBoard.ts` / `dipRadar.ts` → `tests/test_sector_board_snapshot.py` / `test_theme_board_snapshot.py` / `test_dip_radar.py`；行为与 API 见本文「市场板块」。主题板块后台刷新线程在 `theme_board_snapshot.theme_board_refresh_loop`，由 `lifespan.py` 启动（env `FUND_AI_THEME_BOARD_REFRESH_*`）；刷新 `refresh_theme_board_snapshot()`（缓存 `theme:boards:v3`）。
