...
- 荐基按估算持仓市值计算既有板块暴露；已知现金封顶预算，现金明确为 0 时不给金额，现金未单录时以用户本次明确预算封顶。行情、申赎、持仓快照或关键交易门禁不可靠时继续 fail-closed。
...
- `fund_vehicle_quality.2026-07.v1` 将基金级证据按类型拆开：被动/指数基金使用已核验的精确跟踪或主板块身份、申赎、规模、费率与跟踪质量；主动基金使用管理业绩、历史波动背景、规模与资料完整性。机会优先下回撤分固定为中性证据值，不奖励低波动；被动基金的板块绝对涨跌和板块回撤不进入载体质量分；`sector_fit_score` 始终是独立硬门，不能在质量分中重复加分。
