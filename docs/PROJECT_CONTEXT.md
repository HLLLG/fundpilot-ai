...
**文档版本：** 2026-07-25（前端首屏 CSS 拆分、渲染热点治理、静态层预压缩与前端性能门禁）

**本轮验证：** Web 全量 **105 files / 471 passed**（与基线一致）、TypeScript typecheck、ESLint（`--max-warnings=0`）、production 静态导出、首屏体积预算门禁与 `git diff --check` 全部通过。Playwright UI smoke 为 **24 passed / 6 failed / 6 skipped**（合计 36；6 个 skipped 是视口条件跳过）：6 个失败全部是 `visual-regression` 快照，且用 `git stash` 在同机重建 HEAD 后失败**同样的项、失败像素数逐一完全相同**（42464 / 12713 / 12617 / 143343 / 7262 / 3872 / 27423 / 2119 / 2071），证明本轮改动在公共路由上与 HEAD 逐像素一致，快照基线相对本机 Chromium 已陈旧；本轮未更新基线图，是否统一在 CI 生成待决策。首屏关键资源 `/` 从 **629.8 KiB 降至 607.2 KiB**（CSS 148.9 → 125.0，6 条路由同等受益；JS +1.3），gzip 170.8 → 167.3 KiB。交互延迟（INP 同口径最大 `event` duration）3 次采样一致地从 **20～24 ms 降至 16 ms** 单帧下限，代价是面板 DOM 出现墙钟 +约 27 ms（`useDeferredValue` 的显式取舍）。本轮未动后端代码与 API 契约。第二轮（放开框架与视觉约束后）实测否掉三个框架级方案：Next 16.2.10 已无 `clientSegmentCache` 开关（217.4 KiB App Router 运行时无配置出口）、`experimental.inlineCss` 会把 `/` 的 HTML 从 9.9 涨到 262.2 KiB 且丢掉 immutable 缓存、`reactCompiler` 首屏 JS +6.7 KiB / 全量 +156.1 KiB 而交互指标零变化；落地的是 `.factor-bar-fill` 由 `transition: width` 改为 `clip-path`（去逐帧重排、圆头不变）、加载态改为壳层骨架屏、体积预算改按 `firstScreenBytes`（HTML+JS+CSS）卡以挡住"把 CSS 内联进 HTML"这类假收益、交互基准新增 layout-shift 来源捕获。完整实现、测量口径、评估结论与回滚方式见 `docs/perf/web_frontend_20260725.md`。
