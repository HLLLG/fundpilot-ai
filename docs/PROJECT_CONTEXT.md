...
**更新记录：**
- **主题板块改小倍式精选白名单（2026-06-18）：** 东财 `m:90 t:2/t:3` 含 ~500 细分行业/概念（钨、磨具磨料…）过碎；改为对标小倍养基「今日板块涨幅榜」的**固定粗粒度白名单**（`theme_board_snapshot._THEME_BOARD_WHITELIST`，~64 个粗板块：人工智能/消费电子/半导体/稀土/创新药/软件/银行/保险…），每个名经 canonical → 别名 `_THEME_BOARD_ALIAS`（软件→软件开发、基建→基础建设、化工→基础化工、农业→农林牧渔、保险→保险Ⅱ 等）→ 东财概念/行业**精确名**解析到 secid，解析不到（港股/指数类）跳过，实得 **~62 个**干净粗板块。其余口径（push2delay 日 K 同源算 change+streak、后台 15min 刷新、board_kind 标签、持仓高亮）不变。
...
| GET | `/api/market/theme-boards` | 主题板块（**小倍式粗粒度精选白名单 ~62**：人工智能/消费电子/半导体/稀土/创新药/软件/银行/保险…，非东财 500 细分）：默认 `sort=change`；每行含 `board_kind`；**涨跌幅 + 连涨天数同源 push2delay 日 K**（后台 `refresh_theme_board_snapshot`、`allow_akshare=False`）；日 K 缺失用 spot f3 兜底涨跌幅；含我的持仓高亮（canonical secid）；响应含 `refreshed_at`；后台 daemon 15min/1h 刷新、前台只读缓存；clist 抓取 push2delay 优先；`force_refresh` 强制刷新 |
