...
**文档版本：** 2026-07-25（前端首屏 CSS 拆分、渲染热点治理、静态层预压缩与 brotli、前端性能门禁、用户可见错误文案收敛）
...
- 方向成熟度 `sector_entry_maturity.2026-07.v2` 把“中期方向”和“今天能否入场”分开：方向潜力 = 45% 相对强度 + 40% 趋势持续性 + 15% 市场结构；形态成熟度 = 50% 标准化资金强度 + 25% 上涨广度 + 15% 趋势 + 10% 市场结构；入场成熟度 = 35% 方向潜力 + 40% 形态成熟度 + 25% 价格结构。缺失证据不重分权重，方向研究分封顶 45。
- 状态机：`ready_to_start` 要求方向/形态/入场至少 55/55/60、主线 forming/confirmed、资金为正或拐点确认且不过热；`ready_on_pullback` 表示方向仍强但短期加速/拥挤；`forming` 只观察；`invalid` 表示资金派发、5/20 日同时转负或结构破坏。
- 只有 `ready_to_start` 且基金质量、申赎、费用、净值、预算、集中度和组合风险全部通过时，Guard 才能校正为分批买入；首批不超过本次可用预算 20%，后续批次必须重新扫描。页面允许诚实显示“今日可布局方向 0 个”。这些阈值是初始确定性策略，不是已证明的收益最优参数。
