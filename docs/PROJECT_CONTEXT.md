...
**本轮验证：** Web 全量 **105 files / 471 passed**（与基线一致）、TypeScript typecheck、ESLint（`--max-warnings=0`）、production 静态导出、首屏体积预算门禁与 `git diff --check` 全部通过。Playwright UI smoke 为 **27 passed / 6 failed / 3 skipped**：6 个失败全部是 `visual-regression` 快照，且用 `git stash` 在同机重建 HEAD 后失败**同样的项、失败像素数逐一完全相同**（42464 / 12713 / 12617 / 143343 / 7262 / 3872 / 27423 / 2119 / 2071），证明本轮改动在公共路由上与 HEAD 逐像素一致，快照基线相对本机 Chromium 已陈旧；本轮未更新基线图，是否统一在 CI 生成待决策。首屏关键资源 `/` 从 **629.8 KiB 降至 607.2 KiB**（CSS 148.9 → 125.0，6 条路由同等受益；JS +1.3），gzip 170.8 → 167.3 KiB。交互延迟（INP 同口径最大 `event` duration）3 次采样一致地从 **20～24 ms 降至 16 ms** 单帧下限，代价是面板 DOM 出现墙钟 +约 27 ms（`useDeferredValue` 的显式取舍）。本轮未动后端代码与 API 契约。完整实现、测量口径、评估结论与回滚方式见 `docs/perf/web_frontend_20260725.md`。
