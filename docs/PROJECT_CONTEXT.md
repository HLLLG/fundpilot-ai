...
| 首页看板 | **持有** Tab：`YangjibaoHoldingsBoard` 养基宝式卡片（`AddHoldingModal` 上传支付宝/养基宝截图）；**localStorage 缓存优先** instant 展示 → `GET /api/portfolio/holdings`（服务端 120s 内存缓存 + `refreshed_at`）→ 后台自动 refresh-sector-quotes；点击行打开 `YangjibaoFundDetail` |
| 导航记忆 | Dashboard 主 Tab（持有/盈亏分析/市场/推荐基金/生成日报）与 **市场** 子 Tab（主题板块/美股）均用 `sessionStorage` 刷新后恢复 |
...
| 市场板块 | **市场** Tab：`MarketTab` — 子 Tab「**主题板块 \| 美股**」；主题：`ThemeSectorOverview`（`GET /api/market/theme-boards`，**小倍式精选白名单 ~66**、`board_kind` 标签、涨跌幅+连涨 **push2delay 日 K**、**主力净流入+四档展开**、列头排序；后台 15min 刷新、前台只读缓存） |
...
| CI / E2E | GitHub Actions：`api` 并行 pytest（**301** 项，~1min 量级）+ `web` lint/typecheck/build + Playwright 冒烟 |
...
│   ├── lib/api.ts / auth.ts / storage.ts / clientCache.ts / useCachedFetch.ts / portfolioHoldingsCache.ts / marketThemeBoard.ts / holdingMetrics.ts / useSectorQuoteRefresh.ts / workflowBlockers.ts
│   └── components/
│       ├── AuthProvider.tsx       # JWT 与 /api/auth/me
│       ├── Dashboard.tsx          # 持有 / 盈亏分析 / 市场 / 推荐基金 / 生成日报 / 历史（Tab sessionStorage）
...
| 项 | 说明 |
|----|------|
| 规模 | **301** 项单元测试（自 ~400+ 精简；去掉重复集成测与纯网络拉取测） |
...
18. 改市场板块：`sector_board_snapshot.py` / `theme_board_snapshot.py` / `sector_daily_kline_provider.py` → `main.py` `/api/market/*` → `MarketTab.tsx` / `api.ts` / `marketThemeBoard.ts` → `tests/test_sector_board_snapshot.py` / `test_theme_board_snapshot.py`；设计见 `2026-06-17-market-*-design.md`、`2026-06-18-market-theme-boards-expansion-design.md`、`2026-06-18-theme-board-capital-flow-design.md`。主题板块后台刷新线程在 `theme_board_snapshot.theme_board_refresh_loop`，由 `lifespan.py` 启动（env `FUND_AI_THEME_BOARD_REFRESH_*`）；刷新 `refresh_theme_board_snapshot()`（缓存 `theme:boards:v3`）。
...
| `docs/superpowers/specs/2026-06-17-market-sector-performance-design.md` | 全市场板块 API（**Web UI 已移除**，缓存仍供主题板块资金流） |
| `docs/superpowers/specs/2026-06-17-market-theme-boards-design.md` | 市场 Tab — 主题板块（养基宝口径，历史） |
| `docs/superpowers/specs/2026-06-18-market-theme-boards-expansion-design.md` | 市场 Tab — 主题板块白名单 ~66 + 后台 15min 刷新 |
| `docs/superpowers/specs/2026-06-18-theme-board-capital-flow-design.md` | 主题板块主力净流入 + 四档展开 + BK 双映射 |
