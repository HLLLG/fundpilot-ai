...
**文档版本：** 2026-07-21（文档归一化、现行契约、分时排障与 Lighthouse 生产拓扑）

**本轮验证：** 功能回归基线为 API 全量 **1323 passed**、Web 全量 **104 files / 462 passed**，TypeScript typecheck、源码 ESLint、production build、Python `compileall`、本地 Docker Compose 配置与 `git diff --check` 均通过。文档归一化后复核 `docs/` **3 份**、已删除路径残留 **0**、本地 Markdown 链接全部有效；分时诊断聚焦 **4 passed**，真实只读探针同时通过 spot / intraday（`2.931994` 返回 240 点）。Web 全量测试使用单工作进程，规避 Windows 上 Vitest IPC 工作进程的非业务性偶发关闭。
...
- canonical 身份必须精确：中证电网设备 `2.931994`、中证人工智能主题 `2.930713`、中证人工智能产业 `2.931071`、中证半导体 `2.931865`、商业航天概念 `90.BK0963`。中证指数通常使用 `2.{code}`，概念板块使用 `90.BKxxxx`；禁止凭近似名称互换代码。
- 直连分钟数据优先 `push2delay`，再尝试少量 `push2` 子域；有效曲线至少 30 个可解析点。若整批 `|percent|` 最大值仍小于 `0.1`，按“小数值被误当百分比”的污染数据拒绝；少于 30 点的开收盘骨架同样不写缓存、不作 stale 回退。当前缓存命名空间是 `intraday:v3:`，旧 `v2` 记录不会被读取。
...
缓存恢复优先使用 `force_refresh=true`。只有确认 `intraday:v3:` 中存在手工导入或旧版本遗留的异常记录时，才在备份数据库后精确检查并删除对应 `sector_spot_cache.cache_key`；不得清空业务表或使用旧文档中的 `intraday:v2:` 清理语句。变更 `sector_canonical.py`、`eastmoney_trends_client.py` 或浏览器命令后，必须运行 `tests/test_sector_quote_diagnostic.py`，再对受影响的真实板块做上述端点自测。
