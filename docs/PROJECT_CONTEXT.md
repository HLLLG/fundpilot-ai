...
- 面向用户只保留“市场优选”和“组合补缺”。LLM 不从市场自由编造基金，也不能把观察项升级为买入；候选必须先经确定性补数、家族去重、质量门、交易门和量化白名单，再由模型解释。
...
- 线上现行方向口径是 `sector_entry_maturity.2026-08.v3`：三个**正交**分块，每个原始分量只进入一次。趋势强度 = 55% 相对强度 + 45% 趋势持续性；资金参与度 = 60% 标准化资金强度 + 40% 上涨广度；价格位置 = 市场结构分本身。市场结构不再奖励低波动、浅回撤或机械贴近高点，而按20日修复率、离低点反弹、近5日方向、上涨日和量能计算。方向分 = 70% 趋势强度 + 15% 资金参与度 + 15% 价格位置。UI 与 Prompt 都不得把这三块描述为“三重确认”。
...
- 阈值 `V3_GATE_THRESHOLDS` 为趋势 60 / 参与度 35 / 位置 25。参与度 35～55 的离线结果几乎无差，本轮按高弹性目标取35；position 只作结构破坏下限，并允许基金自身修复信号在严格条件下替代。
...
- 资金横截面分位使用按板块自身体量归一化的 `normalized_5d_net` / `normalized_20d_net`（累计净流入 ÷ 近20日 |日主力净流入| 均值 × 天数），并按 `flow_universe`（东财 BK 板块 vs 主题指数 f62 聚合）分池排名。禁止用绝对亿元做跨板块排名：那会让自由流通市值大一个量级的板块机械性占据资金分位榜首。离线回测实测该修正把资金分量的 Rank IC 由显著为负（-0.10/-0.08/-0.06）翻为正（+0.06/+0.08/+0.06）。
...
- v3 状态机：`ready_to_start` 要求趋势≥60、主线 forming/confirmed/crowded、参与度≥35、位置≥25；`crowded` 只触发更低首批比例，不构成方向否决。趋势够但其余不够 → `ready_on_pullback`；趋势不够 → `forming`；双弱/退潮/结构破坏 → `invalid`。若 ready_on_pullback 的唯一失败项是板块位置，基金自身 `fund_entry_signal.entry_ready=true` 可替代位置项；资金或趋势失败时绝不可替代。v2 状态机仅用于解读历史报告。
...
- 只有 `ready_to_start`（或严格满足基金修复替代位置项）且基金质量、申赎、费用、净值、预算、集中度和组合风险全部通过时，Guard 才能校正为分批买入；v3 的确定性分配上限仍为 `20% × first_tranche_scale`，基金修复替代同样适用该上限。每只高弹性买入候选必须附确定性修复失效退出复核条件；这不是成交价保证。这些阈值是产品策略参数，不是已证明的收益最优参数。
