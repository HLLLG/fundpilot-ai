...
**文档版本：** 2026-08-03（基金当日涨跌分布与后台预热）

**方向 V3 本地验收（尚未提交/推送/部署）：** API 全量 **1450 passed**；Web 全量 **106 files / 480 passed**；TypeScript typecheck、ESLint、production build、首屏体积预算、Python `compileall` 与 `git diff --check` 通过。`data/app.db` 的同源 BK 缓存以零网络模式分别重放 V2/V3：68 个 A 股方向、40 个决策日、2675 个观测；V3 报告正确标记 `sector_entry_maturity.2026-08.v3` 并逐交易日应用滞回，T+5/10/20 相对缓存板块等权基准的去均值超额为 **+2.71% / +4.13% / +6.50%（均显著）**；V2 同口径为 **+0.22% / -0.49% / +1.56%（ready 桶均仅 28 个决策日，不显著）**。该结果只描述 2026-01～07 的缓存板块指数，不等于基金可实现收益，也不取得自动调参或自动晋级资格。
...
- `excluded`：规模 <0.5 亿元或成立不足 1 年；`watch_only`：规模 0.5～1 亿元、核心字段不全、NAV 超过 7 个自然日、近一年最大回撤 >50%，或双源刷新失败只能用陈旧档案；只有显式 `eligible` 才可能买入。未知门禁、重复基金、否定买入文案、非有限金额或零预算一律清金额并降观察。
- `fund_vehicle_quality.2026-07.v1` 将基金级证据按类型拆开：被动/指数基金使用已核验的精确跟踪或主板块身份、申赎、规模、费率与跟踪质量；主动基金使用管理业绩、回撤控制、规模与资料完整性。被动基金的板块绝对涨跌和板块回撤不进入载体质量分；`sector_fit_score` 始终是独立硬门，不能在质量分中重复加分。
...
- 线上现行方向口径是 `sector_entry_maturity.2026-08.v3`：三个**正交**分块，每个原始分量只进入一次。趋势强度 = 55% 相对强度 + 45% 趋势持续性；资金参与度 = 60% 标准化资金强度 + 40% 上涨广度；价格位置 = 市场结构分本身（不再叠加任何回调/贴高点加减分）。方向分 = 70% 趋势强度 + 15% 资金参与度 + 15% 价格位置，权重按实测 T+5 Rank IC 比例取整（.338 : .064 : .066）。UI 与 Prompt 都不得把这三块描述为“三重确认”。
...
- v3 的过热是**风险披露而非否决**：实测 `overheated=True` 的方向前瞻超额为 +2.75%（t=+5.23, T+5）/ +5.31%（t=+3.99, T+20），而 False 为 -0.49% / -0.95%——v2 把最强的正向信号当成了淘汰条件。命中时写入 `overheat_flags` 并按 `first_tranche_scale`（1.0 / 0.6 / 0.4）缩小首批，文案须说明“短期加速、首批更小、不预先承诺后续”。**刻意不把高涨幅、贴高点、远离MA20 反转成加分项**：上述观测来自单一 6 个月动量区间的 40 个决策日，反向押注同样是未经验证的下注；v3 只移除实测有害的先验。
...
- 阈值 `V3_GATE_THRESHOLDS`（趋势 60 / 参与度 45 / 位置 25）经 `scan_entry_gate_thresholds` 实测网格选取，**刻意不取网格最优**（趋势 70 时 T+20 达 +10.19% 但 n 掉到 31，等于平均每天不到 1 个方向）。position 在网格里不起约束作用，只作结构破坏下限，不假装它有区分力。
...
- v3 状态机：`ready_to_start` 要求趋势≥60、主线 forming/confirmed/crowded、参与度≥45、位置≥25；`crowded` 只触发更低首批比例，不构成方向否决。趋势够但其余不够 → `ready_on_pullback`（含义是“趋势仍强，资金参与度或价格位置尚不支持立即入场”，不再由过热触发）；趋势不够 → `forming`；双弱/退潮/结构破坏 → `invalid`。v2 状态机（55/55/60/50 + 过热硬排除 + 5 日资金不为净流出）仅用于解读历史报告。
- `sector_direction_state.v1` 引入跨日滞回：进入 `ready_to_start` 需连续 2 个交易日通过入场线（首次达标当天显示“已满足 1 天”且不给买入动作）；退出使用更低的趋势线（入场线 −8）形成滞回带，已确认的方向只有跌破退出线才降级，但 `invalid` 永远立即生效。连续天数按交易日相邻判断，上一交易日无记录时从 1 重新起算，不假装中间那些天也达标。滞回只改变状态的时序行为，不改变任何分数、不放宽入场线；无历史时完全不生效，绝不变成额外一道门。状态按 (trade_date, sector_label) 幂等落 `sector_direction_states`，写失败只丢失滞回能力、不影响当次结论。
- 方向状态必须在**选择之前**套上滞回：`entry_state` 是排序的第一优先级，先选 8 个再改状态会让展示状态与入选依据变成两套东西。
- 只有 `ready_to_start` 且基金质量、申赎、费用、净值、预算、集中度和组合风险全部通过时，Guard 才能校正为分批买入；v2 首批不超过本次可用预算 20%，v3 的确定性分配上限为 `20% × first_tranche_scale`，多方向共用组合级额度时取最小 scale，后续批次必须重新扫描。页面允许诚实显示“今日可布局方向 0 个”。这些阈值是初始确定性策略，不是已证明的收益最优参数。
