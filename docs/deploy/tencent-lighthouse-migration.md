...
`.env.production` 示例。所有“替换为”都必须填入真实值；数据库密码使用 `openssl rand -hex` 的值，避免 URL 编码问题。
    ...
    WEB_CONCURRENCY=2
    FUND_AI_BACKGROUND_WORKER_LOCK_TIMEOUT_SECONDS=5
    FUND_AI_BACKGROUND_WORKER_RETRY_SECONDS=5
    FUND_AI_BACKGROUND_WORKER_HEARTBEAT_INTERVAL_SECONDS=10
    FUND_AI_BACKGROUND_WORKER_HEARTBEAT_STALE_SECONDS=45
    FUND_AI_PORTFOLIO_MUTATION_LOCK_TIMEOUT_SECONDS=30
...
schema v17 另要求 `factor_ic_nav_observations` 满足精确列、唯一内容哈希、查询索引、InnoDB 和
UPDATE/DELETE 无条件拒绝触发器契约；它保存采集器真实首次看到的 NAV，不能用历史值回填。
...
自动发布会在替换 API 容器前运行 `python -m app.mysql_admin_bootstrap`。该一次性步骤使用
`MYSQL_ROOT_PASSWORD` 创建或校验表、索引和不可变触发器，随后 API 仍使用
`FUND_AI_DATABASE_URL` 中的低权限账号运行。发布容器只共享 MySQL 的网络命名空间并连接
`127.0.0.1`，管理员端口不会暴露到 Docker 网络。不得为了发布成功而跳过触发器契约或让
API 长期使用 root。发布失败时，脚本会按 `/srv/fundpilot/DEPLOYED_SHA` 自动恢复上一健康版本。
API 镜像按 Git 提交标记为 `fundpilot-api:<sha>`，新镜像通过全部健康检查前不会覆盖上一健康镜像。
...
在 `/srv/fundpilot/repo/docker-compose.production.yml` 写入：
    ...
    services:
      ...
      worker:
        image: ${FUND_AI_API_IMAGE:-fundpilot-api:latest}
        restart: unless-stopped
        env_file: .env.production
        environment:
          FUND_AI_PROJECT_ROOT: /app
          FUND_AI_RUNTIME_ROLE: worker
        ...
        depends_on:
          mysql:
            condition: service_healthy
        healthcheck:
          test: ["CMD", "python", "-m", "app.background_worker", "--healthcheck"]
          interval: 10s
          timeout: 5s
          retries: 12
          start_period: 15s
...
恢复测试不能只核对业务表行数。恢复后先启动一次 API 让 bootstrap 验真，再执行：
    ...
    docker compose --env-file .env.production -f docker-compose.production.yml exec -T mysql mysql -u root -p"$MYSQL_ROOT_PASSWORD" "$MYSQL_DATABASE" -e "SHOW CREATE TABLE factor_ic_nav_observations; SHOW INDEX FROM factor_ic_nav_observations;"
...
`factor_ic_nav_observations` 还必须为 InnoDB，具有完整内容哈希唯一索引、按基金/净值日/观测时间
查询的非前缀索引，以及 UPDATE/DELETE 两枚无条件拒绝触发器。API bootstrap 日志中不得出现
质量账、NAV 观测契约或 SQLite fallback 错误。
