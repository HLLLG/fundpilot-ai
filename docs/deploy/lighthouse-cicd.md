...
3. `Decision Outcome Settlement`：手动运行一次，确认只处理正式 pending event；之后每天 23:35 自动结算日报 T+1/5/20 与荐基 T+5/20/60。该任务复用 `LIGHTHOUSE_DEPLOY_ENABLED=true` 和同一组 SSH secrets，不需要发布 Token。
