...
1. `Factor IC Universe Capture`：发布当天 1,500 个不可变研究成员；工作日 20:30 自动运行。先在 MySQL 核对 `factor_ic_universe_snapshots` 和 `factor_ic_universe_members`，不得只看 Actions 绿色状态。
2. `Factor IC Refresh`：有成员快照后再运行周度研究。首次仍可能发布/保留 v2，并显示“PIT 积累中”；这是正确降级，不得降低 v3 门槛。
3. `Decision Outcome Settlement`：手动运行一次，确认只处理正式 pending event；之后每天 23:35 自动结算新日报/荐基 T+5/20/60，并继续兼容历史日报 T+1 pending。该任务复用 `LIGHTHOUSE_DEPLOY_ENABLED=true` 和同一组 SSH secrets，不需要发布 Token。

当前 Factor IC v3 固定拉取完整基金目录、去重并分层回测 1,500 个独立组合，同时有界读取最多 180 个历史成员快照；workflow 上限 90 分钟。Actions Summary 必须同时显示源目录/去重/有效组合覆盖、成员 PIT 状态及六类基金的统计/经济门槛；仅显示 job success 但没有 `result: created|duplicate` 不算验收完成。随着历史成员并集扩大，应监控 NAV 拉取成功率和运行时长，后续用 append-only NAV observation 缓存做增量取数，不削减样本或放宽门槛。
...
当前 v3 只冻结基金池成员，`point_in_time_scope=membership_only`、`nav_revision_pit=false`。普通基金按 1 个交易日、QDII 按 2 个交易日的 NAV 发布滞后研究；状态显示“成员PIT”不代表完整 NAV-PIT。按 10 日再平衡步长，20 日经济门槛理论最短约 17.5 个月、60 日约 19.5 个月，实际还要通过 FDR、区间、折数一致与净成本门槛。
