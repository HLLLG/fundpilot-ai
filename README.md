...
| 模块 | 做什么 |
|------|--------|
| **持仓台账** | 上传支付宝「全部持有 / 我的持有」（含三列版）或养基宝总览截图，预览确认后写入账户汇总；也可粘贴 OCR 文本或手动录入 |
| **导入交易** | 支付宝「交易分析」截图写入买卖流水，不当作同步持仓。进行中交易先以上看板（不计收益、不加总资产），确认日净值公布后再入账。卖出显示「份」视为全部卖出：当日仍留在持仓页算收益，次日从账本删除 |
...
持仓看板按养基宝式展示代码、金额、持有 / 当日收益和板块涨跌。主动基金的关联板块来自季报重仓穿透，被动基金跟合同指数；研究线索不会写成当前主板块。盘中当日收益按关联板块估算（标 ≈）；真实份额、成本、现金与费用未录入时保持 unknown，不按 0 猜测。
...
3. 预览确认后写入账户汇总。进行中交易先出现在看板，确认日净值公布后再计入持仓。卖出显示「份」视为全部卖出，当日仍留在持仓页算收益，次日从账本删除。
