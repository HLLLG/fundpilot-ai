...
- 养基宝总览 OCR：识别多只基金、区分当日/持有收益（未收盘时当日列为 `-` 则不填当日收益）；**OCR 漏负号**时按收益率/账户总收益/独立行减号规则补符号（当日收益额、板块涨跌与收益率一致）。详见 `docs/design/2026-06-01-portfolio-holdings.md`。
- 持仓校对表支持基金代码、名称、金额、持有/当日收益、板块涨跌；**估算当日收益率**（≈ 板块涨跌 + 持有收益率，与传给模型一致）。
...
- 基金档案支持 **JSON 导入/导出**；风控参数、分析模式、追问模式保存在浏览器本地。
...
2. 上传养基宝总览截图或粘贴 OCR 文本 → 校对持仓（可看「估算当日收益率」列）
3. 选择快速/深度分析模式 → 点击「生成报告」
4. 右下角悬浮面板查看进度 → 完成后进入「分析」页查看日报
5. 在「决策建议」右侧追问助手继续提问（可选深度模式拉最新新闻）
6. 需要留存时可导出日报 Markdown 或导出对话 Markdown
```
...
本项目面向个人自用。截图、数据库和上传文件默认保存在本地项目目录。DeepSeek 会收到你确认后的持仓、风控参数、净值摘要、已生成日报全文（追问时），以及经 AkShare 从公开渠道拉取的新闻标题/摘要，不会自动上传原始截图。报告与对话只用于个人投研辅助，不构成投资建议，也不会执行任何交易。
