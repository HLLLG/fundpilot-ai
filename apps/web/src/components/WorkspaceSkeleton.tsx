 ...
 *
 * 这里按工作台真实壳层的尺寸给出骨架：顶栏（4.25rem）+ 主内容卡片。
 * 真实工作台替换它时，外层容器、内边距与顶栏高度一致，所以不会产生新的位移。
 *
 * 注意：**只使用首屏 CSS 里已有的 Tailwind utilities**，不使用 `app-masthead` /
 * `dashboard-shell` 这些类。那些规则住在按需加载的 `dashboard.css` 里，
 * 如果骨架屏依赖它们，就会把工作台样式重新拉回首屏，让 CSS 拆分白做，
 * 而且骨架自己会先闪一帧无样式。
 */
export function WorkspaceSkeleton({ message }: { message: string }) {
  return (
    <div className="premium-bg min-h-screen">
      <div
        className="mx-auto flex min-h-screen w-full max-w-[1240px] flex-col px-4 pb-24 sm:px-6 lg:pb-6"
        role="status"
        aria-live="polite"
        aria-busy="true"
      >
