...
export function translateEvidenceText(text: string): string {
  return text
    ...
    .replace(/持仓与字段级证据未达到可执行条件，本次已禁止仓位动作。?/g, "关键持仓或行情数据未达到时点可用条件，因此暂不提供加减仓操作。")
    .replace(/字段级证据时点校验未通过，仓位动作已被确定性阻断。?/g, "关键信息完整性与更新时间校验未通过，系统已暂时关闭仓位操作。")
    .replace(/量化证据未覆盖该候选，系统仅保留观察，不以未验证因子支持买入。?/g, "量化模型暂时没有给这只基金加分；旧版稳健策略因此先列为观察。").replace(/候选未进入当前量化\s*finalists，须待净值时点与同类因子校验完成后再评估。?/gi, "量化模型尚未完成这只基金的同类比较，当前没有量化加分依据。")
    .replace(/部分候选未进入当前量化覆盖集合，已降为观察并清除买入金额。?/g, "部分候选暂时没有量化模型加分；这份旧版稳健报告因此将其列为观察。").replace(/与您(?:保守的)?风险偏好严重不符/g, "历史波动较大，首批仓位应相应降低")
