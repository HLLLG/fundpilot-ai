...
test("公共落地页具备清晰主路径与关键语义", async ({ page }) => {
  ...
  const nav = page.getByRole("navigation", { name: "账号入口" });
  await expect(nav.getByRole("link", { name: "登录" })).toHaveAttribute("href", "/login");
  await expect(nav.getByRole("link", { name: "注册" })).toHaveAttribute("href", "/register");

  const primaryCta = page.getByTestId("landing-primary-cta");
  await expect(primaryCta).toHaveAccessibleName("注册");
