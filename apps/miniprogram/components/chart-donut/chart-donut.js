// components/chart-donut/chart-donut.js
// 甜甜圈/饼图业务封装，包裹 ec-canvas（官方 echarts 桥接组件）。
//
// 用法：
//   <chart-donut option="{{donutOption}}" canvas-id="alloc-chart" />
//
// option 由 utils/derive.js 的 buildDonutChartOption 生成，
// 组件只负责把 option 传给 echarts 实例的 setOption。
//
// Requirements: 9.7（Chart_Renderer 展示持仓分布甜甜圈图）
// Design: miniprogram-web-parity / 图表方案 / chart-donut 业务封装
...
Component({
  properties: {
    ...
    canvasId: {
      type: String,
      value: 'chart-donut-canvas',
    },
    // 图表高度（rpx 字符串，默认 400rpx）。
    height: {
      type: String,
      value: '400rpx',
    },
