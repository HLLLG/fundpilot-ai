...
背景：反向代理通常会在 SSE 响应连接持续空闲超过一定时长后主动中断连接，
客户端表现为 ``ERR_ABORT_HANDLER`` / ``net/http: abort Handler``。
深度推理模型在返回首个 token 前可能有较长的思考耗时，一旦超过空闲阈值，
即使后端仍在正常工作，连接也会被强制断开，导致「AI 分析中…」阶段偶发卡死/报错。
