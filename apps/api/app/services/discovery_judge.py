...
_RISK_REVIEW_TASK_PROMPT_ENFORCED = (
    ...
    "任务：\n"
    "1. 对草案中「建议关注/等待回调」的推荐做入场复核。只有板块 entry_state=ready_to_start，"
    "或 V3 ready_on_pullback 且基金自身 fund_entry_signal.entry_ready=true、满足系统允许的"
    "结构修复替代条件，或 flow_improving_probe_eligible=true 且基金自身入场信号通过，或"
    " probability_early_probe_eligible=true 且基金 entry_ready/early_probe_ready=true 时，才可"
    "升级为「分批买入」；后两条只能缩小本次投入。机会分、selection_priority_score、"
    "selection_path=high_elasticity、高置信度或基金质量分本身都不能越过入场状态。\n"
