...
_INDUSTRY_TO_THEME: dict[str, str] = {
    ...
    "中药Ⅱ": "中药",
    "医疗服务": "医疗",
    # 申万三级；医院/体检仍停在上一级「医疗服务」→ 医疗，不并进 CXO。
    "医疗研发外包": "CXO",
    # ---- 有色 / 钢铁 / 化工 ----
    "工业金属": "有色金属",
    "小金属": "有色金属",
    ...
    "工程机械": "机械设备",
    "轨交设备Ⅱ": "机械设备",
    # 申万「国防军工」一级下正好 5 个二级行业，必须整组落到军工，缺一个就会把军工
    # 基金的持仓票数拆到别的板块去。航海装备Ⅱ 曾被放进上面的机械设备组（大概是跟
    # 轨交设备Ⅱ 混了），但它装的是中国船舶 / 中船防务 / 松发股份(恒力重工) 这些军
    # 船标的——已按数据源 f127 实测确认：这三只报的都是「航海装备Ⅱ」，而中国中车
    # 报的是「轨交设备Ⅱ」，两者不同父级。
    "航海装备Ⅱ": "军工",
    "航空装备Ⅱ": "军工",
    "航天装备Ⅱ": "军工",
