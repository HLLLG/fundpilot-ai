...
def test_vlm_ocr_settings_defaults(monkeypatch):
    ...
    assert s.vlm_ocr_max_pixels == 2_000_000
    assert s.vlm_ocr_compress_enabled is True
    assert s.vlm_ocr_jpeg_quality == 80
    assert s.vlm_ocr_max_image_side == 1600


def test_local_ocr_settings_are_gone(monkeypatch):
    """本地 PaddleOCR 已删除，相关开关不该再存在（否则是配置了但没人读的死旋钮）。"""
    s = refresh_settings()

    for removed in (
        "ocr_provider",
        "ocr_preload",
        "ocr_use_mobile_models",
        "ocr_max_image_side",
    ):
