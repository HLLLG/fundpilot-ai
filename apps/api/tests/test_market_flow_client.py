...
def test_stock_connect_parser_exposes_only_southbound():
    result = service._parse_stock_connect_summary(_rows(), ANCHOR)

    assert result is not None
    assert not any("northbound" in key for key in result)
    assert result["southbound_net_yi"] == 12.5
    assert result["southbound_available"] is True
    assert result["interpretation"] == "南向净流入约 12 亿，可作为港股资金面的独立参考。"
...
def test_unavailable_context_exposes_only_southbound_status(monkeypatch):
    monkeypatch.setattr(service, "fetch_stock_connect_flow_summary", lambda *_a, **_k: None)

    result = service.build_stock_connect_flow_context(ANCHOR)

    assert result["available"] is False
    assert result["schema_version"] == "stock_connect_flow.v2"
    assert not any("northbound" in key for key in result)
