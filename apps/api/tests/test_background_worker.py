...
def _disabled_settings(**overrides):
    values = {
        ...
        "fund_nav_series_refresh_enabled": False,
...
def test_holding_market_cache_job_registers_when_warmup_enabled(monkeypatch) -> None:
    monkeypatch.setattr(
        background_worker,
        "get_settings",
        lambda: _disabled_settings(
            runtime_role="worker",
            holding_intraday_warmup_enabled=True,
        ),
    )
...
def test_nav_series_refresh_job_registers_when_enabled(monkeypatch) -> None:
    monkeypatch.setattr(
        background_worker,
        "get_settings",
        lambda: _disabled_settings(
            runtime_role="worker",
            fund_nav_series_refresh_enabled=True,
        ),
    )

    jobs = background_worker.configured_background_jobs()

    assert [job.name for job in jobs] == ["fund-nav-series-refresh"]
...
def test_sqlite_worker_acquires_leadership_and_serves_healthcheck(
    ...
    monkeypatch.setenv("FUND_AI_FUND_NAV_SERIES_REFRESH_ENABLED", "false")
    refresh_settings()
