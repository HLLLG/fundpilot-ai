...
_MANUAL_INDEX_CODES: dict[str, tuple[str, str | None]] = {
    ...
    # 930997 被两条 AMAC 全称抢用（中证智能电动汽车 / 中证新能源产业），至多一条能对。
    # 归智能电动汽车，依据是有真实产品作证：华宝中证智能电动汽车ETF 联接
    # (013475/013476) 的业绩基准就是"中证智能电动汽车指数"，而它记录的跟踪码正是
    # 930997；930997 的东财实况简称是"新能源车"，与 THEME_BOARD_INDEX["新能源车"]
    # 同码同名，整条链自洽。中证新能源产业指数在库里没有任何基金跟踪，它才是查不到的那个。
    #
    # 这里必须给出一个码，不能图省事写 (None, None)：`parse_benchmark_index` 在拿不到
    # 库里代码时**不会** fail-closed，而是退回按名字做子串匹配，"中证智能电动汽车指数"
    # 会命中更粗的 931008 汽车 —— 那两只 ETF 联接会从"新能源车"掉成"汽车"。
    "中证智能电动汽车指数": ("930997", "新能源车"),
    "中证新能源产业指数": (None, None),
    "中信汽车指数": ("817018", None),
